...
void GlrMyNode::render ( SnShape* s, GlContext* ctx )
{
	...
	{	//GsPnt o = c.init;
		...
		float x, y, z;
		float n = c.resolution;
		GsVec pointA, pointB, pointC, pointD;

		if ( r<=0 || R<=0 ) return; // invalid parameters

		GsArray<GsVec> P(0,6); // will hold the points forming my triangles (size 0, but pre-allocate 6 spaces)
		//P.push() = o;

		for (phi = 0.0f; phi < gs2pi; phi = phi + n) {
			for (theta = 0.0f; theta < gs2pi; theta = theta + n) {
					//first point A with (theta, phi)
					...
					//second point B with (theta, phi + delta phi)
					x = float((R + (r * cos(theta))) * cos(phi + n));
					y = float((R + (r * cos(theta))) * sin(phi + n));
					z = float(r * sin(theta));
					pointB = GsVec(x, y, z);
					P.push() = pointB;

					//third point C with (theta+ delta theta, phi+ delta phi)
					x = float((R + (r * cos(theta + n))) * cos(phi + n));
					y = float((R + (r * cos(theta + n))) * sin(phi + n));
					z = float(r * sin(theta + n));
					pointC = GsVec(x, y, z);
					P.push() = pointC;
					
					//fourth point which is the same as the first
					P.push() = pointA;

					//fifth point which is the same as the third
					P.push() = pointC;

					//last point which is (theta + delta theta, phi)
					x = float((R + (r * cos(theta + n))) * cos(phi));
					y = float((R + (r * cos(theta + n))) * sin(phi));
					z = float(r * sin(theta + n));
					pointD = GsVec(x, y, z);
					P.push() = pointD;
			}
